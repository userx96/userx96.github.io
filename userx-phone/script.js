$(document).ready(() => {
  window.onload = function() {
    ...
    const note_delete_btn = document.getElementById("note-element-delete-btn")
    ...
    const goNote = (e) => {
      ...
      const not_content = note.children[3].textContent

      const note_element_container = document.getElementById("note-element-container")
      note_element_container.classList.add("right-fix")
      const note_element_title = document.getElementById("note-element-title")
      const note_element_content = document.getElementById("note-element-content")
    ...
    const noteDelete = (e) => {
      ...
      const text = note.children[1].children[0].textContent
      ...
      const note_element_container = document.getElementById("note-element-container")
      note_element_container.classList.remove("right-fix")
      note_element_container.classList.add("right")

      // localden siliyor fakat html silemedim
    }

    note_delete_btn.addEventListener("click", noteDelete)
  ...
  pass_check.onchange = () => {
    if (pass_check.checked) {
      ...
      $("#password-create-container").css({display: "flex"}).animate({top: "155px"})
    } else {
...
volume_range.addEventListener("change", volumeChanger) 