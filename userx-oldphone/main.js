...
function notEkle() {
  ...
  if (not == "" || not_baslik == ""){
    ...
    $("#not-app-notes").prepend("<li id='not-1'><span id='not-title'>" + not_baslik + "</span><span id='not-desc'>"+ "<span id='not-saat'>"+ addZero(hours)+ ":"+ addZero(mins) +"</span>" +"<span id='not-day'>"+gunler[gun]+"</span>" + " " + not + "</span> <span id='not-delete'></span></li>");
    $(" #not-icerik-container").fadeOut(100);
    $(".backButton").fadeIn(100);
...
function msgGonder() {
  ...
  var wpbildirim = document.getElementById("wp-bildirim");
...
// burası
  else if (mesaj == "amk" || mesaj == "aq" || mesaj == "orrrospu") {
    $("#msg").append("<li class='giden-mesaj'> <span id='giden-msg-text'>"+ mesaj +"</span> <span id='giden-msg-saat'>15:09</span><span id='okunmadi'></span><span id='okundu'></span> </li>").delay(1000).queue(function(next1){
      wpbildirim.play();
      $("#msg").append("<li class='gelen-mesaj'> <span id='gelen-msg-text'>"+ "ORROSPUNUN SIÇTIĞI" +"</span> <span id='gelen-msg-saat'>15:09</span> </li>");
      next1();
...
function msgBildirim() {
  msgBildirim.stop();
   var kilit = document.querySelector(".kilitAc");
   var bildirim = document.getElementById("bildirim-sesi");
   setTimeout(function(){
      $(".msg-bildirim-container").fadeIn(200).animate({
        height: "60px"
      },200).delay(3000).animate({
        height: "0px"
      })
      bildirim.play();
      $("#msg").append("<li class='gelen-mesaj'> <span id='gelen-msg-text'>"+ "Merhaba Kanka" +"</span> <span id='gelen-msg-saat'>15:09</span> </li>");
   },3000)
   
}
$(".msg-bildirim-container").click(function(){
  $(this).hide();
  $(".whatsappContainer").show();
  $(".sohbet1message").fadeIn(200);
});
